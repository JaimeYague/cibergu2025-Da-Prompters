...
SENSITIVE_PATTERNS = [
    ...
    (r"\b[A-Z]{4}[A-Z]{2}[A-Z0-9]{2}([A-Z0-9]{3})?\b", "### SWIFT/BIC ###"),
    ...
    (r"\b\d{8}[a-hj-np-tv-zA-HJ-NP-TV-Z]\b", "### DNI ###"),
    (r"\b[XYZxyz]\d{7}[A-Za-z]\b", "### NIE ###"),
    (r"\b[abcdefghjklmnpqrsuvwABCDEFGHJKLMNPQRSUVW]\d{7}[0-9A-Ja-j]\b", "### NIF ###"),
    (r"\b\d{2}\s?\d{8,10}\b", "### SEG. SOCIAL ###"),
    (r"\b\d{2}[- ]?\d{8,10}\b", "### NAF ###"),
    (r"\b\d{10}\b", "### SIP ###"),
    (r"\b[A-Za-z]{3}\d{6}\b", "### PASAPORTE ###"),
    (r"\b[XYZxyz][- ]?\d{7}[- ]?[A-Za-z]\b", "### NIE (VAR) ###"),
    ...
    (r"\b[a-zA-Za-z0-9]{24,128}\b", "### TOKEN/API KEY ###"),
    (r"-----BEGIN [A-Za-z ]{5,50}KEY-----[\s\S]+?-----END [A-Za-z ]{5,50}KEY-----", "### CLAVE PRIVADA ###"),
    (r"\b\d{4,8}\b", "### CÓDIGO ###"),
    ...
    (r"\b(Calle|Avda|Avenida|C/|Plaza|Paseo|Camino|Carretera)\s+[\w\s]+,\s*\d{1,5}(?:[\s,]+[A-Za-zÁÉÍÓÚÑa-záéíóúñ\s]+)?", "### DIRECCIÓN ###"),
    ...
    (r"\b(\d{10,16}[A-Za-z]?)\b", "### TARJETA SANITARIA ###"),
    ...
    (r"\b(AIza[0-9A-Za-z\-_]{35}|sk_live_[0-9a-zA-Za-z]{24}|sk_test_[0-9a-zA-Za-z]{24}|AKIA[0-9A-Za-z]{16})\b", "### CLAVE API ###"),
